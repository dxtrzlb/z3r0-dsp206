...
### Phase E — Frequency-view UI (redesign)
Replaces the original channel-strip grid. The **Frequency View** (interactive EQ + crossover
graph) is the center of the app; routing, output editor, meters, and safety all orbit it. Layout:
top bar (preset / USB / **Mute All** / Edit·Safety·Party switch); always-on Output Overview Bar
(O1–O6, named, metered, click-to-select); left rail (Inputs + Routing matrix with a SUM A+B row);
center Frequency View; below, a tabbed Output Editor (XOVER · EQ · LIMITER · DELAY, with
Compressor/Gate under an advanced sub-tab; GEQ replaces the editor for inputs). No PHASE tab —
the protocol exposes no polarity command. The existing `store.ts` + `commands.ts` already cover
every frame; this phase is presentation only.
- **Checkpoint per control:** adjusting it emits the frame that byte-matches the doc (thin test)
  and the live-verifiable ones move the hardware.

### Phase F — Safety/Party views, presets, packaging
- **Safety Mode** (fullscreen): per-output SAFE/WARNING/CLIPPING from meter thresholds; peak;
  limiter activity **inferred** from level-vs-threshold and thermal a heuristic — both labeled
  "estimated", never presented as device telemetry (the protocol reports levels only).
- **Party Mode** (fullscreen): large read-only monitor for across-the-tent visibility.
...
- **Checkpoint:** clip state flips near 0 dBFS; preset load verified after disconnect; package builds.

> Undo: the pre-redesign plan is preserved in `PLAN.backup.md` / `TASKS.backup.md`.
